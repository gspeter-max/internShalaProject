...
from multiagent.tools.custom_tool import ( 
    ...
    getCurrentTime,
    getPreviousSentContent,
    WriteFileAndCreateTool,
    ReadFileAndSummarizeTool,
    getUpdateFromUser
    ) 
import os
import torch 
...
class Multiagent():
    ...
    def Researcher(self) -> Agent:
        return Agent(
            ...
            multimodal = True,
            inject_data = True,
            tools = [ SerperDevTool(),getUpdateFromUser(),getCurrentTime()],
            llm = self.llm
        )
    ...
    def SummaryWriter(self) -> Agent:
        return Agent(
            ...
            respect_context_window= True,
            llm = self.llm,
            tools =[SendTextInTelegramChannel(),WriteFileAndCreateTool(),ReadFileAndSummarizeTool()]
        )
    ...
    def Visualizer(self) -> Agent:
        return Agent(
            ...
            respect_context_window= True,
            llm = self.llm,
            tools = [ ConvertPromptToTerminalCommand() ,SendImageInTelegramChannel(),ReadFileAndSummarizeTool(),TerminalExecution() , ImageGenerationTool()]
        )
